...
## 共同帧误判与恢复目标固定（2026-09-17）

静态边界 fixture 发现：分别 nearest(T) 再检查 A/B 差值会漏掉可用帧对。
例如 A 有 T−30ms、T+35ms，B 有 T+30ms；旧算法选择 −30/+30，差值 60ms
而冻结，尽管 +35/+30 是符合所有 40ms 限制的共同帧。冻结后 T 不推进，
同一误选可能一直持续到超时重锚。这是代码可复现的恢复循环，不代表已确认现场唯一根因。

现在在 T±40ms 内联合选择 A/B 候选，先最小化最大 T 误差，再最小化总误差；
A/B 差值仍不超过 40ms，每侧 presentedRt 不倒退。单侧降级沿用同一选择接口。
等待时固定首次失败的候选 T，不再因 rAF 间隔抖动而反复改变解码目标；
共同帧恢复并通过滞回后清除固定目标。换源、成员切换、权威切换、重锚会清理它。
持续缺帧仍按既有规则重锚，不靠放宽误差或取消恢复掩盖问题。

新增 fixture 覆盖误选、回调抖动、恢复后不额外 seek、误差/单调边界、
单侧播放及 81 组候选相位组合。未做真流复现或现场性能验证。
