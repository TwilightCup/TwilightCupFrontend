  ...
  <div class="home">
    <header class="head">
      <div class="brand">
        ...
        <div>
          <div class="title">{{ $t("referee.title") }}</div>
          <div class="subtitle">{{ $t("referee.subtitle") }}</div>
        </div>
      </div>
      <div class="actions">
    ...
    <div v-loading="loading" class="list">
      ...
      >
        <div class="card-main">
          ...
          <div class="meta">
            <span>{{ $t("referee.matchMeta", { bo: s.bo_format, win: s.win_threshold }) }}</span>
            <span class="vs">
              <span class="tc-a">{{ s.player_a_name }}</span>
              <span class="dim"> vs </span>
              <span class="tc-b">{{ s.player_b_name }}</span>
            </span>
            <span class="dim">{{ $t("referee.refereeLabel", { name: s.referee_name }) }}</span>
            <span class="dim">{{ dateTime(s.created_at) }}</span>
          </div>
        </div>
        <div class="card-op">
          <router-link :to="`/referee/${s.id}`" class="enter" @click.prevent="go(s)">
            {{ $t("referee.enterMatch") }}
          </router-link>
