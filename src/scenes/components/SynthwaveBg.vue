 ...
 *
 * default / synthwave 为纯 CSS/SVG，无外部资源（OBS 浏览器源离线缓存可靠）；
 * synthwave_gl「水面浪潮GL」使用内联 GLSL 的全屏 WebGL 重绘（见 SynthwaveGlBg.vue），
...
import SynthwaveGlBg from "@/scenes/components/SynthwaveGlBg.vue";
...
/** 新版全屏 WebGL 水面预设。 */
const isWaterGl = computed(() => background.value === "synthwave_gl");
/** WebGL 不可用 / 上下文丢失时置位，自动回退到 CSS/SVG 水面。 */
const shaderUnavailable = ref(false);
/** 实际渲染预设：GL 失败时映射到旧版 synthwave。 */
const renderBackground = computed(() =>
  isWaterGl.value && shaderUnavailable.value ? "synthwave" : background.value,
);
/** 当前是否由 CSS/SVG 渲染水面（旧版 synthwave，或 GL 失败后的回退）。 */
const isWaterCss = computed(() => renderBackground.value === "synthwave");
...
// 背景切回 GL 时重置失败标记；只有实际走 CSS/SVG 水面分支时才运行 SVG 置换 rAF。
watch(
  background,
  () => {
    if (isWaterGl.value) shaderUnavailable.value = false;
  },
  { immediate: true, flush: "post" },
);
watch(
  [background, shaderUnavailable],
  () => {
...
<template>
  <div class="synthwave-bg" :data-background="renderBackground" aria-hidden="true">
    <!-- synthwave_gl：全屏 WebGL 重绘；失败时回退到下面的 CSS/SVG 水面 -->
    <SynthwaveGlBg
      v-if="isWaterGl && !shaderUnavailable"
    ...
    <template v-else>
      ...
      <!-- 随机缓慢闪烁星星（仅水面版） -->
      <div v-if="isWaterCss" class="stars">
      ...
      <div v-if="isWaterCss" class="water">
      ...
      <div v-if="!isWaterCss" class="horizon-glow" />

      <!-- 顶部/底部压暗，贴近参考图的暗角 -->
      <div v-if="isWaterCss" class="vignette" />
    </template>
  </div>
</template>
