...
export class ExternalClock {
  private anchor: { t: number; at: number; rate: number; received: number } | null = null;
  ...
  accept(p: FrameAlignAnchor, now: number): boolean {
    ...
    if (p.rate != null && (!Number.isFinite(p.rate) || p.rate < 0 || p.rate > 2)) return false;
    ...
    if (changed && (src !== this.allowedSource || this.seenSources.has(src!))) return false;
    const newerEpoch = p.epoch != null && (this.epoch == null || p.epoch > this.epoch);
    if (!changed && !newerEpoch && this.seq != null && (p.seq == null || p.seq <= this.seq)) return false;
    ...
    this.seq = p.seq ?? null;
    const rate = p.paused ? 0 : p.rate ?? (p.t_us === this.lastInput ? 0 : 1);
    this.anchor = { t: p.t_us, at: now - age, rate, received: now };
  ...
  read(now: number): { t: number; rate: number; stale: boolean } | null {
    ...
    const a = this.anchor;
    const stale = now - a.received > 1500;
    // Bound speculative travel when the authority disappears, then freeze permanently.
    const elapsed = Math.max(0, Math.min(now - a.at, 1000));
