...
const props = withDefaults(
  defineProps<{
    side: Side;
    /** Native-resolution director rendering with identity deduplication and visibility gating. */
    directorOutput?: boolean;
...
const panelVisible = props.directorOutput ? usePanelVisibility(cv) : null;
...
const hasImage = computed(() => !alignEngine.pictureExpired.value && alignEngine.hasCanvasImage(cv.value));
  ...
  <div class="frame" :class="[side, { uncropped: !props.crop4to3 }]">
    <div v-if="!props.hidden" class="stage">
      ...
      <div v-if="!hasImage" class="ph-abs">
        ...
        <template v-if="bare || signalWaiting || alignEngine.pictureExpired.value">
