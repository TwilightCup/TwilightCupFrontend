 ...
 * 当前尝试实时分段时间（live_time segment），下行 = 「上一次次数/总次数」+
 * 上一次尝试用时（跳过/未完成显示 N/A）；沿用同一两行副计时几何。两行标签
 * 盒外缘对齐线条外缘、盒宽
...
<template>
  <div class="timer" :class="side">
    ...
    <!-- 副计时器两行：多关上行 = 当前关卡名 + 实时单段（白），下行 = 上一关 +
         单段用时（淡紫，PB 卡弱化色）；单关上行 = 「当前次数/总次数」+ 当前
         尝试实时单段，下行 = 「上一次次数/总次数」+ 上一次尝试用时（跳过显示
         N/A）。任一元素缺省降为透明占位（NBSP 保基线 + 满量程宽，行高恒定，
         主计时器与另一行位置不偏移） -->
    <div class="subs">
